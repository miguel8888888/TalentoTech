...
class GestionMatriculadosController extends Controller
{
    /**
    ...
    {
        $request->flash();
          // Utilizamos el método where para filtrar por estado_registro igual a "Matriculado"
        $participantes = Participante::where('estado_registro', 'Matricula')
        ->where(function ($query) use ($request) {
            // Agregamos la lógica de búsqueda si se proporciona algún término de búsqueda
            if ($s = $request->buscar) {
                $query->where('primer_nombre', 'LIKE', '%' . $s . '%')
